...
#if  _MSC_VER >= 1400 // vs2005+
	_tmakepath_s(szPath, _MAX_PATH, szDrive, szDir, IFO_DFT_FILE_NAME, IFO_DFT_FILE_EXT);
#else
	_tmakepath(szPath, szDrive, szDir, IFO_DFT_FILE_NAME, IFO_DFT_FILE_EXT);
#endif

	m_strIniFilePath = szPath;
}

CIniFileOperations::CIniFileOperations(LPCTSTR lpszFilePath)
{
	m_strIniFilePath = lpszFilePath;
}

CIniFileOperations::CIniFileOperations(LPCTSTR lpszFileName, LPCTSTR lpszFileExt)
{
	TCHAR szPath	[_MAX_PATH];
	TCHAR szDrive	[_MAX_DRIVE];
	TCHAR szDir		[_MAX_DIR];
...
#if  _MSC_VER >= 1400 // vs2005+
	_tmakepath_s(szPath, _MAX_PATH, szDrive, szDir, lpszFileName, lpszFileExt);
#else
	_tmakepath(szPath, szDrive, szDir, lpszFileName, lpszFileExt);
#endif
